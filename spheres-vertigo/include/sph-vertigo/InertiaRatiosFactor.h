 ...
 * NOTE(tonioteran)
 */
class InertiaRatiosFactor
    : public NoiseModelFactor6<Vector2, Vector3, Pose3, Pose3, Pose3, Pose3> {
 private:
  typedef NoiseModelFactor6<Vector2, Vector3, Pose3, Pose3, Pose3, Pose3> Base;
 ...
 public:
   ...
   */
  InertiaRatiosFactor(const Key& J, const Key& h_ij, const Key& T_GiBi,
                      const Key& T_GjBj, const Key& T_WBi, const Key& T_WBj,
                      const SharedNoiseModel& model, const Rot3& R_BG,
   ...
   */
  Vector evaluateError(const Vector2& J, const Vector3& h_ij,
  ...
  bool equals(const InertiaRatiosFactor& factor, double tol = 1e-9) const;
...
template <>
struct traits<InertiaRatiosFactor> : public Testable<InertiaRatiosFactor> {};

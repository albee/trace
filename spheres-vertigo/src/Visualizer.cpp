...
#include "sph-vertigo/Visualizer.h"
...
void Visualizer::RenderWorld() {
  ...
  labels.push_back(std::string("OmegaX_G"));
  labels.push_back(std::string("OmegaY_G"));
  labels.push_back(std::string("OmegaZ_G"));
  omegaxLog_.SetLabels(labels);
  ...
  double theta = 0.0, dtheta = 0.0005;
  ...
  while (!pangolin::ShouldQuit()) {
    ...
    // TEMP
    auto initview = pangolin::ModelViewLookAt(
        std::cos(theta), std::sin(theta), 1.0, 0.0, 0.0, 0.0, pangolin::AxisZ);
    s_cam.SetModelViewMatrix(initview);
    // TEMP
    ...
    // size_t delay_plot = 0;
    size_t delay_plot = 150;
    if (rotomegas_.size() > delay_plot) pangolin::glDrawPoints(rotomegas_);
    // Draw line connecting all measurements.
    glColor4f(0.65, 0.45, 0.9, 0.5);
    glLineWidth(1.0);
    if (rotomegas_.size() > delay_plot) pangolin::glDrawLineStrip(rotomegas_);
    glColor3f(1.0, 1.0, 1.0);

    // Draw projections.
    DrawPolhodePlaneProjections();

    DrawGyroPolhode();

    // TEMP
    double rPolhode = 1.8;
    auto viewPolhode = pangolin::ModelViewLookAt(
        rPolhode * std::cos(theta), rPolhode * std::sin(theta), 0.5, 0.0, 0.0,
        0.0, pangolin::AxisZ);
    s_cam_polhode.SetModelViewMatrix(viewPolhode);
    // TEMP
    ...
    // TEMP
    double rSync = 2.0;
    auto viewSync = pangolin::ModelViewLookAt(rSync * std::cos(theta),
                                              rSync * std::sin(theta), 0.8, 0.0,
                                              0.0, 0.0, pangolin::AxisZ);
    s_cam_isync.SetModelViewMatrix(viewSync);
    // TEMP
...
void Visualizer::UpdateEstimate(const gtsam::Values& values,
  ...
  // Export file for building motion models.
  std::ofstream outMM("omegaRealDataMM.txt");
  ...
  for (int j = 1; j < num_poses; j++) {
    ...
    if (deltaT < 0.8) {
      ...
      // Output to file for building motion models.
      // Get average rotation for including as corresponding pose.
      Eigen::Vector3d avgTheta = (0.5) * (Log(Ri) + Log(Rj));
      Eigen::Matrix3d avgR = box::ExpMapRot(avgTheta);
      outMM << avgTime << " " << omega.transpose() << " " << avgR(0, 0) << " "
            << avgR(1, 0) << " " << avgR(2, 0) << " " << avgR(0, 1) << " "
            << avgR(1, 1) << " " << avgR(2, 1) << " " << avgR(0, 2) << " "
            << avgR(1, 2) << " " << avgR(2, 2) << std::endl;
    }
  }
...
/* *************************************************************************  */
void Visualizer::DrawPolhodePlaneProjections() const {
  // --- XY.
  glPointSize(5.0);  // Default is 1.
  glColor3f(0.7, 0.7, 0.7);
  if (xyproj_.size()) pangolin::glDrawPoints(xyproj_);
  // Draw line connecting all measurements.
  glColor4f(0.75, 0.75, 0.7, 0.5);
  glLineWidth(1.0);
  pangolin::glDrawLineStrip(xyproj_);
  glColor3f(1.0, 1.0, 1.0);

  // --- XZ.
  glPointSize(5.0);  // Default is 1.
  glColor3f(0.7, 0.7, 0.7);
  if (xzproj_.size()) pangolin::glDrawPoints(xzproj_);
  // Draw line connecting all measurements.
  glColor4f(0.75, 0.75, 0.7, 0.5);
  glLineWidth(1.0);
  pangolin::glDrawLineStrip(xzproj_);
  glColor3f(1.0, 1.0, 1.0);

  // --- YZ.
  glPointSize(5.0);  // Default is 1.
  glColor3f(0.7, 0.7, 0.7);
  if (yzproj_.size()) pangolin::glDrawPoints(yzproj_);
  // Draw line connecting all measurements.
  glColor4f(0.75, 0.75, 0.7, 0.5);
  glLineWidth(1.0);
  pangolin::glDrawLineStrip(yzproj_);
  glColor3f(1.0, 1.0, 1.0);
}

...
void GraphManager::InitChaserChain(const Eigen::Matrix4f &T_WC, const Eigen::Vector3f &v_WC, const Eigen::Vector3f &t_GC) {
    ...
    gtsam::PriorFactor<gtsam::Point3> blobPrior(blob_sym, blob0, params_.blobpNM);
...
void GraphManager::AddFactors(const Eigen::Matrix4f &T_CiCj, const int frame_idx,
    ...
    double range = centroid.norm() + params_.blob_range_bias;
    gtsam::Unit3 bearing(centroid.cast<double>());
    ...
    mit_slam::RotationKinematicFactor rkf(pose_i, geom_i, pose_j, geom_j, t_geom, params_.rkfNM);

...
class Polytope:
  ...
  def _set_Ab(self, A, b):
    ...
    m, n = A.shape
    if b.shape[0] != m:
      raise ValueError('A has {m} rows; b has {b.shape[0]} rows!')
  ...
  def _set_Ab_from_bounds(self, lb, ub):
    ...
    if ub.size > 0:
      if not ub.size == n:
        raise ValueError(('Dimension of upper bound ub is f{ub.size}; '
                          'should be {n}'))
      A_bound.extend(np.eye(n))
      b_bound.extend(ub)
  ...
  def __str__(self):
    return type(self).__name__ + ' in R^{self.n}'
  ...
  def __numpy_ufunc__(self, *args):
    # used to force proper rmul multiplication with np
    pass
  ...
  def contains(self, Q):
    ...
    if isinstance(Q, Polytope):  # P and Q are both polytopes
      ...
      if x.shape[0] == 1:  # make x a column if it is single point (here a row)
        ...
        raise ValueError('Testing if a polytope in R^{self.n} contains points'
                         'in R^{x.shape[0]} is not allowed: points must be '
                         'columns (transpose the input?)')
...
def P_plus_p(P, point, subtract_p=False):
  ...
  if p.size != P.n or p.shape[1] != 1 or p.ndim != 2:  # ensure p is n x 1
    raise ValueError('The point must be a vector in R^{P.n}')
...
def minkowski_sum(P, Q):
  ...
  if P.n != Q.n:
    raise ValueError('Minkowski sum of polytopes of different dimensions '
                     '({P.n} and {Q.n}) not allowed')
...
def pontryagin_difference(P, Q):
  ...
  if P.n != Q.n:
    raise ValueError('Pontryagin difference of polytopes of different '
                     'dimensions ({P.n} and {Q.n}) not allowed')
  m = P.A.shape[0]
...
def redundant_inequalities(A, b):
  ...
  for ineq in range(m):
    ...
    if not lp_result.success:
      print('Tested inequality {ineq}: {lp_result.message}')
...
def solve_lp(c, solver='linprog', *args, **kwargs):
  ...
  if solver.lower() == 'linprog':
    kwargs['method'] = 'simplex'
    result = linprog(c, **kwargs)
    if not result.success:
      print({result.message})
  else:
    raise NotImplementedError('Support for solver {solver} not implemented')
  return result

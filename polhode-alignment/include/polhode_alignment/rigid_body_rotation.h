/**
 * @file rigid_body_rotation.h
 * @brief Object to simulate rigid body dynamics.
 * @author Antonio Teran (antonioteran@google.com)
 * Copyright 2021 Antonio Teran
 */

#ifndef POLHODE_ALIGNMENT_RIGID_BODY_ROTATION_H_
#define POLHODE_ALIGNMENT_RIGID_BODY_ROTATION_H_
...
namespace pao {
  ...
  using PlaneFit = std::pair<ConicFit<double>, int>;
/**
...
class RigidBodyRotation {
 public:
  ...
  std::vector<std::pair<ConicFit<double>, int>> GetBestConicFitsConstrained(
      const Eigen::Matrix3d& R, const Eigen::MatrixXd& omegaB_G ) const;
...
#endif  // POLHODE_ALIGNMENT_RIGID_BODY_ROTATION_H_
